...
public class AttackControlScript : Photon.MonoBehaviour
{
    ...
    //The firing list.  
    List<TileAttackScript> selectedTiles = new List<TileAttackScript>(3);

    //Arrays of the ships.
    GameObject[] shipObjects;
    ShipAttackScript[] shipScripts;
    ...
    void Start()
    {
        ...
        shipScripts = new ShipAttackScript[3];
        for (int i = 0; i < shipObjects.Length; i++)
        {
            shipScripts[i] = shipObjects[i].GetComponent<ShipAttackScript>();
        }
    ...
    void ProcessInputEvent()
    {
        ...
        if (Physics.Raycast(ray, out hit, 100))
        {
            if (hit.collider.tag == "Tile")
            {
                ...
                TileAttackScript ts = hit.collider.GetComponent<TileAttackScript>();
    ...
    void DetermineShots()
    {
        ...
        List<ShipAttackScript> activeShips = new List<ShipAttackScript>(3);

        //Loop through all ship scripts and add them to the list.
        foreach (ShipAttackScript ss in shipScripts)
        {
            if (ss.isAlive)
            {
                activeShips.Add(ss);
    ...
    public void ForceEndTurn()
    {
        ...
        foreach (TileAttackScript ts in selectedTiles)
        {
            //Deselct the tile.
            ts.DeselectTile();
        }
    ...
    void HitShip(string shipName)
    {
        ...
        ShipAttackScript ss = GameObject.Find(shipName).GetComponent<ShipAttackScript>();
    ...
    {
        //Reveal the ship.
        GameObject.Find(shipName).GetComponent<ShipAttackScript>().Reveal();
    }
